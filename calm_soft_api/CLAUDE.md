...
Publish **SPF (incl. Hostinger's send hosts) + DKIM + DMARC** for `calmsoft.pro` before relying on delivery. Use a dedicated, minimally-scoped SMTP account (not the whole-domain mailbox). Point uptime monitoring at `GET /ready` (runs a live `transporter.verify()` **per probe**, bounded by the transport timeouts — a boot-time flag would always lose the race on lsnode's cold-process-per-request lifecycle), not `GET /health` — `/health` stays green even when SMTP is dead. Alert on `503`s from `POST /api/contact` and on `mail send failed` log lines, via a channel **other than** Hostinger SMTP. (The in-memory send-cap counter resets per boot on lsnode, so a cap-approach alert cannot fire outside a sustained flood.)

- **Required post-deploy step:** submit the live contact form end-to-end and confirm the internal notification email actually arrives at `MAIL_TEAM_TO`. With the synchronous-send design this is the only way to prove delivery works — there is no outbox left to inspect afterward.
